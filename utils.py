    ...
    if 'scaler' in pipelines[model_name].named_steps:
        ...
        if 'param_scaler' in model_search.cv_results_:
            ...
            #     indices_scaler = [i for i, param in enumerate(model_search.cv_results_['param_scaler']) if str(param) == scaler_name]
                ...
                indices_scaler = [
                    i for i, param in enumerate(model_search.cv_results_['param_scaler']) 
                    if (param is None and scaler_class is None) or (scaler_class is not None and isinstance(param, scaler_class))
                ]

                if indices_scaler:
                    # Melhor índice para o scaler específico
                    best_index_scaler = indices_scaler[np.argmax(model_search.cv_results_['mean_test_g_mean'][indices_scaler])]
                    best_score_scaler = model_search.cv_results_['mean_test_g_mean'][best_index_scaler]
                    std_score_scaler = model_search.cv_results_['std_test_g_mean'][best_index_scaler]  # Desvio padrão para a métrica de teste
                    best_score_scaler_train = model_search.cv_results_['mean_train_g_mean'][best_index_scaler]
                    std_score_scaler_train = model_search.cv_results_['std_train_g_mean'][best_index_scaler]

                    # Armazenar os resultados no formato adequado para exportação
                    results.append({
                        'Model': model_name,
                        'Scaler': scaler_name,
                        'Best Params': model_search.cv_results_['params'][best_index_scaler],
